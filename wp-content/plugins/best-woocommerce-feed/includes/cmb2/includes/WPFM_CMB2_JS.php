<?php
/**
 * Handles the dependencies and enqueueing of the WPFM_CMB2 JS scripts
 *
...
class WPFM_CMB2_JS {
	...
	/**
	 * Array of WPFM_CMB2 JS dependencies
	 *
	 * @var   array
	 * @since 2.0.7
	 */
	...
	/**
	 * Array of WPFM_CMB2 fields model data for JS.
	 *
	 * @var   array
	 * @since 2.4.0
	 */
	...
	/**
	 * Add a dependency to the array of WPFM_CMB2 JS dependencies
	 *
	 * @since 2.0.7
	 ...
	 */
	public static function add_field_data( WPFM_CMB2_Field $field ) {
		$hash = $field->hash_id();
	...
	public static function enqueue() {
		...
		if ( isset( $dependencies['media-editor'] ) ) {
			wp_enqueue_media();
			WPFM_CMB2_Type_File_Base::output_js_underscore_templates();
		}
		...
		// Enqueue cmb JS
		wp_enqueue_script( self::$handle, WPFM_CMB2_Utils::url( "js/cmb2{$min}.js" ), $dependencies, WPFM_CMB2_VERSION, true );

		// if SCRIPT_DEBUG, we need to enqueue separately.
		if ( $enqueue_wysiwyg ) {
			wp_enqueue_script( 'cmb2-wysiwyg', WPFM_CMB2_Utils::url( 'js/cmb2-wysiwyg.js' ), array( 'jquery', 'wp-util' ), WPFM_CMB2_VERSION );
		}
	...
	public static function register_colorpicker_alpha( $enqueue = false ) {
		...
		$func( 'wp-color-picker-alpha', WPFM_CMB2_Utils::url( "js/wp-color-picker-alpha{$min}.js" ), array( 'wp-color-picker' ), '2.1.3' );
	}
	...
	public static function register_datetimepicker( $enqueue = false ) {
		$func = $enqueue ? 'wp_enqueue_script' : 'wp_register_script';
		$func( 'jquery-ui-datetimepicker', WPFM_CMB2_Utils::url( 'js/jquery-ui-timepicker-addon.min.js' ), array( 'jquery-ui-slider' ), '1.5.0' );
	}
	...
	protected static function colorpicker_frontend() {
		wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), WPFM_CMB2_VERSION );
		wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), WPFM_CMB2_VERSION );
	...
	/**
	 * Localize the php variables for WPFM_CMB2 JS
	 *
	 * @since  2.0.7
	 */
