...
class WPFM_CMB2_Sanitize {
	 ...
	 */
	public function __construct( WPFM_CMB2_Field $field, $value ) {
		$this->field = $field;
		$this->value = $value;
	}
	...
	public function taxonomy() {
		...
		if ( ! $this->field->args( 'taxonomy' ) ) {
			WPFM_CMB2_Utils::log_if_debug( __METHOD__, __LINE__, "{$this->field->type()} {$this->field->_id()} is missing the 'taxonomy' parameter." );
		} else {
			 ...
			 * (as they are stored as terms). This allows overriding that and is used by WPFM_CMB2::get_sanitized_values().
			 *
			 * The dynamic portion of the hook, $cmb_id, refers to the this field's WPFM_CMB2 box id.
			 *
			 * @since 2.2.4
			 *
			 ...
			 * @param WPFM_CMB2_Sanitize $sanitizer This object.
			 */
	...
	public function text_money() {
		...
		// See https://github.com/WPFM_CMB2/WPFM_CMB2/issues/1014
	...
	public function text_datetime_timestamp_timezone( $repeat = false ) {
		...
		if ( empty( $tzstring ) ) {
			$tzstring = WPFM_CMB2_Utils::timezone_string();
		}

		$offset = WPFM_CMB2_Utils::timezone_offset( $tzstring );
		...
		try {
			...
			$this->value = '';
			WPFM_CMB2_Utils::log_if_debug( __METHOD__, __LINE__, $e->getMessage() );
		}
	...
	public function _save_file_id_value( $file_id_key ) {
		...
		if ( $this->value && ! $id_val ) {
			$id_val = WPFM_CMB2_Utils::image_id_from_url( $this->value );
	...
	/**
	 * Returns a new, supporting, WPFM_CMB2_Field object based on a new field id.
	 *
	 * @since  2.2.0
	 */
