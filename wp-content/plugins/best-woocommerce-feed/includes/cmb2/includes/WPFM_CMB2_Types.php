...
class WPFM_CMB2_Types {
	...
	/**
	 * Current WPFM_CMB2_Field field object
	 *
	...
	/**
	 * Current WPFM_CMB2_Type_Base object
	 *
	 * @var   WPFM_CMB2_Type_Base object
	 * @since 2.2.2
	 */
	public $type = null;
	...
	public function __call( $fieldtype, $arguments ) {
		...
		// Check for methods to be proxied to the WPFM_CMB2_Type_Base object.
		 ...
		 *
		 * @param array  $field              The passed in `WPFM_CMB2_Field` object
		 ...
		 * @param object $field_type_object  This `WPFM_CMB2_Types` object
		 */
	...
	/**
	 * Proxies the method call to the WPFM_CMB2_Type_Base object, if it exists, otherwise returns a default fallback value.
	 *
	 * @since  2.2.2
	 *
	 * @param  string $method  Method to call on the WPFM_CMB2_Type_Base object.
	...
	/**
	 * If no WPFM_CMB2_Types::$type object is initiated when a proxy method is called, it means
	 ...
	 * @since  2.2.3
	 *
	 * @param string $method  Method attempting to be called on the WPFM_CMB2_Type_Base object.
	 */
	protected function guess_type_object( $method ) {
		...
		switch ( $method ) {
			...
			case 'concat_items':
				$this->get_new_render_type( $fieldtype, 'WPFM_CMB2_Type_Select' );
				break;
			case 'is_valid_img_ext':
			case 'img_status_output':
			case 'file_status_output':
				$this->get_new_render_type( $fieldtype, 'WPFM_CMB2_Type_File_Base' );
				break;
			case 'parse_picker_options':
				$this->get_new_render_type( $fieldtype, 'WPFM_CMB2_Type_Text_Date' );
				break;
			case 'get_object_terms':
			case 'get_terms':
				$this->get_new_render_type( $fieldtype, 'WPFM_CMB2_Type_Taxonomy_Multicheck' );
				break;
			case 'date_args':
			case 'time_args':
				$this->get_new_render_type( $fieldtype, 'WPFM_CMB2_Type_Text_Datetime_Timestamp' );
				break;
			case 'parse_args':
				$this->get_new_render_type( $fieldtype, 'WPFM_CMB2_Type_Text' );
				break;
		}
	...
	public function maybe_proxy_method( $method, $arguments ) {
		...
		if ( isset( $proxied[ $method ] ) ) {
			$exists = array(
				// Ok, proxy the method call to the WPFM_CMB2_Type_Base object.
	...
	/**
	 * Checks for a custom field WPFM_CMB2_Type_Base class to use for rendering.
	 *
	 * @since 2.2.4
	 *
	 * @param string $fieldtype Non-existent field type name.
	 ...
	 *
	 * @return bool|WPFM_CMB2_Type_Base Type object if custom field is an object, false if field was added with
	 *                             `cmb2_render_{$field_type}` action.
	 * @throws Exception if custom field type class does not extend WPFM_CMB2_Type_Base.
	 */
	public function maybe_custom_field_object( $fieldtype, $args = array() ) {
		if ( $render_class_name = $this->get_render_type_class( $fieldtype ) ) {
			...
			if ( ! ( $this->type instanceof WPFM_CMB2_Type_Base ) ) {
				throw new Exception( __( 'Custom WPFM_CMB2 field type classes must extend WPFM_CMB2_Type_Base.', 'cmb2' ) );
			}
	...
	/**
	 * Gets the render type WPFM_CMB2_Type_Base object to use for rendering the field.
	 *
	 * @since  2.2.4
	 ...
	 * @return WPFM_CMB2_Type_Base                    Type object.
	 */
	...
	public function get_render_type_class( $fieldtype, $render_class_name = null ) {
		...
		if ( has_action( "cmb2_render_class_{$fieldtype}" ) ) {
			...
			/**
			 * Filters the custom field type class used for rendering the field. Class is required to extend WPFM_CMB2_Type_Base.
			 *
	...
	public function get_file_ext( $file ) {
		return WPFM_CMB2_Utils::get_file_ext( $file );
	}
	...
	public function get_file_name_from_path( $value ) {
		return WPFM_CMB2_Utils::get_file_name_from_path( $value );
	}
	...
	public function concat_attrs( $attrs, $attr_exclude = array() ) {
		return WPFM_CMB2_Utils::concat_attrs( $attrs, $attr_exclude );
	}
	...
	public function input( $args = array(), $type = __FUNCTION__ ) {
		return $this->get_new_render_type( 'text', 'WPFM_CMB2_Type_Text', $args, $type )->render();
	}
	...
	public function textarea( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Textarea', $args )->render();
	}
	...
	public function hidden() {
		...
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text', $args, 'input' )->render();
	}

	public function text_small() {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text', array(
	...
	public function text_money() {
		$input = $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text', array(
	...
	public function textarea_small() {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Textarea', array(
	...
	public function textarea_code( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Textarea_Code', $args )->render();
	}

	public function wysiwyg( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Wysiwyg', $args )->render();
	}

	public function text_date( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text_Date', $args )->render();
	}
	...
	public function text_time( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text_Time', $args )->render();
	}

	public function text_datetime_timestamp( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text_Datetime_Timestamp', $args )->render();
	}

	public function text_datetime_timestamp_timezone( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Text_Datetime_Timestamp_Timezone', $args )->render();
	}

	public function select_timezone( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Select_Timezone', $args )->render();
	}

	public function colorpicker( $args = array(), $meta_value = '' ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Colorpicker', $args, $meta_value )->render();
	}

	public function title( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Title', $args )->render();
	}

	public function select( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Select', $args )->render();
	}

	public function taxonomy_select( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Taxonomy_Select', $args )->render();
	}

	public function radio( $args = array(), $type = __FUNCTION__ ) {
		return $this->get_new_render_type( $type, 'WPFM_CMB2_Type_Radio', $args, $type )->render();
	}
	...
	public function multicheck( $type = 'checkbox' ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Multicheck', array(), $type )->render();
	}
	...
	public function checkbox( $args = array(), $is_checked = null ) {
		...
		$render_class_name = $this->get_render_type_class( __FUNCTION__, 'WPFM_CMB2_Type_Checkbox' );
		$this->type = new $render_class_name( $this, $args, $is_checked );
		return $this->type->render();
	}

	public function taxonomy_radio( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Taxonomy_Radio', $args )->render();
	}

	public function taxonomy_radio_hierarchical( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Taxonomy_Radio_Hierarchical', $args )->render();
	}
	...
	public function taxonomy_multicheck( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Taxonomy_Multicheck', $args )->render();
	}

	public function taxonomy_multicheck_hierarchical( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Taxonomy_Multicheck_Hierarchical', $args )->render();
	}
	...
	public function oembed( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_Oembed', $args )->render();
	}

	public function file_list( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_File_List', $args )->render();
	}

	public function file( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'WPFM_CMB2_Type_File', $args )->render();
	}
