<?php
/**
 * Handles hooking WPFM_CMB2 forms/metaboxes into the post/attachement/user screens
 ...
 * @link      https://cmb2.io
 */
class WPFM_CMB2_hookup extends WPFM_CMB2_Hookup_Base {
	...
	/**
	 * Array of WPFM_CMB2_Options_Hookup instances if options page metabox.
	 *
	 * @var   WPFM_CMB2_Options_Hookup[]|null
	 * @since 2.2.5
	 */
	 ...
	 */
	public static function maybe_init_and_hookup( WPFM_CMB2 $cmb ) {
		if ( $cmb->prop( 'hookup' ) ) {
	...
	public function universal_hooks() {
		foreach ( get_class_methods( 'WPFM_CMB2_Show_Filters' ) as $filter ) {
			add_filter( 'cmb2_show_on', array( 'WPFM_CMB2_Show_Filters', $filter ), 10, 3 );
		}
	...
	public function term_hooks() {
		...
		foreach ( $this->taxonomies as $taxonomy ) {
			 ...
			 * @param object $cmb         The WPFM_CMB2 instance
			 */
	...
	public function options_page_hooks() {
		...
		if ( ! empty( $option_keys ) ) {
			foreach ( $option_keys as $option_key ) {
				$this->options_hookup[ $option_key ] = new WPFM_CMB2_Options_Hookup( $this->cmb, $option_key );
	...
	/**
	 * Registers styles for WPFM_CMB2
	 *
	 * @since 2.0.7
	 */
	protected static function register_styles() {
		...
		wp_register_style( 'cmb2-styles', WPFM_CMB2_Utils::url( "css/cmb2{$front}{$rtl}{$min}.css" ), $dependencies );
		wp_register_style( 'cmb2-display-styles', WPFM_CMB2_Utils::url( "css/cmb2-display{$rtl}{$min}.css" ), $dependencies );

		self::$css_registration_done = true;
	}

	/**
	 * Registers scripts for WPFM_CMB2
	 *
	 * @since  2.0.7
	 */
	protected static function register_js() {
		...
		add_action( $hook, array( 'WPFM_CMB2_JS', 'enqueue' ), 8 );

		self::$js_registration_done = true;
	}

	/**
	 * Registers scripts and styles for WPFM_CMB2
	 *
	 * @since  1.0.0
	 */
	...
	/**
	 * Enqueues scripts and styles for WPFM_CMB2 in admin_head.
	 *
	 * @since  1.0.0
	 */
	...
	/**
	 * Register the WPFM_CMB2 field column headers.
	 *
	 * @since 2.2.2
	 */
	...
	/**
	 * The WPFM_CMB2 field column display output.
	 *
	 * @since 2.2.2
	 */
	public function column_display( $column_name, $object_id ) {
		if ( isset( $this->columns[ $column_name ] ) ) {
				$field = new WPFM_CMB2_Field( array(
	...
	/**
	 * Output the WPFM_CMB2 box/fields in an alternate context (not in a standard metabox area).
	 *
	 * @since 2.2.4
	 */
	...
	public function show_on() {
		 ...
		 * @param mixed  $cmb           The WPFM_CMB2 instance
		 */
	...
	public static function enqueue_cmb_css( $handle = 'cmb2-styles' ) {
		...
		/**
		 * Filter to determine if WPFM_CMB2'S css should be enqueued.
		 *
	...
	public static function enqueue_cmb_js() {
		...
		/**
		 * Filter to determine if WPFM_CMB2'S JS should be enqueued.
		 *
