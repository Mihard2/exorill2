...
class WPFM_CMB2_REST extends WPFM_CMB2_Hookup_Base {
	...
	public static function init_routes() {
		...
		$boxes_controller = new WPFM_CMB2_REST_Controller_Boxes( $wp_rest_server );
		$boxes_controller->register_routes();

		$fields_controller = new WPFM_CMB2_REST_Controller_Fields( $wp_rest_server );
		...
		foreach ( self::$boxes as $cmb_id => $rest_box ) {
			...
			if ( isset( $types['term'] ) ) {
				...
				$taxonomies = array_merge(
					$taxonomies,
					WPFM_CMB2_Utils::ensure_array( $rest_box->cmb->prop( 'taxonomies' ) )
				);
