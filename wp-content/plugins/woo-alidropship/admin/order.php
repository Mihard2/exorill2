...
//		add_filter( 'woocommerce_shop_order_search_fields', array( $this, 'woocommerce_shop_order_search_ali_order' ) );
	...
	public function admin_enqueue_scripts( $page ) {
		...
		if ( $page === 'post.php' ) {
			...
			if ( $screen->id === 'shop_order' ) {
				wp_enqueue_style( 'woo-alidropship-admin-edit-order', VI_WOO_ALIDROPSHIP_CSS . 'admin-order.css' );
				wp_enqueue_script( 'woo-alidropship-admin-edit-order', VI_WOO_ALIDROPSHIP_JS . 'admin-order.js', array( 'jquery' ) );
			...
			wp_enqueue_style( 'woo-alidropship-order-status', VI_WOO_ALIDROPSHIP_CSS . 'order-status.css' );

			wp_enqueue_script( 'woo-alidropship-order-status', VI_WOO_ALIDROPSHIP_JS . 'order-status.js', array( 'jquery' ) );
	...
	public function woocommerce_after_order_itemmeta( $item_id, $item, $product ) {
		...
		if ( $aliexpress_order_id ) {
			...
			$tracking_url_btn = "https://trade.aliexpress.com/orderList.htm?tradeId={$aliexpress_order_id}&getTracking=manual";
		}
		$item_tracking_data    = wc_get_order_item_meta( $item_id, '_vi_wot_order_item_tracking_data', true );
		...
		if ( $col_id === 'order_number' ) {
			...
			if ( count( $order_items ) ) {
				...
				if ( $total && $ali_pid && is_array( $statuses ) && in_array( 'wc-' . $order->get_status(), $statuses ) ) {
					...
					$href   = $aff_url ? "https://www.aliexpress.com?orderID={$order_id}&aliProduct=" . base64_encode( $aff_url ) : "https://www.aliexpress.com/item/{$ali_pid}.html?orderID={$order_id}";
					$target = '_blank';
                            ...
                            <table class="wad-list-ali-order-items" item-id="<?php echo esc_attr( $item_id ) ?>">
                                <tr>
                                    ...
                                    <td class="wad-column">
                                        ...
                                        <a href="https://trade.aliexpress.com/orderList.htm?tradeId=<?php echo esc_attr( $ali_order_id ) ?>&getTracking=manual"
	...
	public function order_ali_button( $order_id ) {
		...
		if ( count( $order_items ) ) {
			foreach ( $order_items as $order_item ) {
				...
				$ali_product_id = get_post_meta( $pid, '_vi_wad_aliexpress_product_id', true );
				$ali_pid        = $ali_product_id ? $ali_product_id : $ali_pid;
				$_aff_url       = get_post_meta( $pid, '_vi_wad_aff_url', true );
				$aff_url        = $_aff_url ? $_aff_url : $aff_url;
			}
		}
		$statuses = $this->settings->get_params( 'order_status_for_fulfill' );
            ...
            <li class="wide">
                <div class="vi-wad-ali-order-btn">
                    <a href="<?php echo esc_url( $href ); ?>" target="_blank" class="button">
						<?php esc_html_e( 'To Order AliExpress', 'woo-alidropship' ); ?></a>
                </div>
            </li>
			<?php
		}
