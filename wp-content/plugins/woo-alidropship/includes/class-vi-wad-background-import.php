	...
	class Vi_WAD_Background_Import_Product extends WP_Background_Process {
		...
		protected function task( $item ) {
			...
			try {
				...
				$aff_url            = isset( $item['aff_url'] ) ? $item['aff_url'] : '';
				...
				if ( is_array( $attributes ) && count( $attributes ) ) {
					...
					if ( ! is_wp_error( $product_id ) ) {
						...
						if ( $image ) {
							...
							VI_WOO_ALIDROPSHIP_Admin_Settings::$process_image->push_to_queue( $image_data );
						}
					...
					if ( ! is_wp_error( $product_id ) ) {
						...
						if ( $dispatch ) {
							VI_WOO_ALIDROPSHIP_Admin_Settings::$process_image->save()->dispatch();
						}
						/*Set product tag*/
						if ( is_array( $tags ) && count( $tags ) ) {
