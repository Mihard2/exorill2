	...
	function vi_wad_upload_image( $url, $post_parent = 0, $exclude = array(), $post_title = '', $desc = null ) {
		...
		if ( is_array( $matches ) && count( $matches ) ) {
			if ( ! in_array( strtolower( $matches[1] ), $exclude ) ) {
				...
				return new WP_Error( 's2w_file_type_not_permitted', esc_html__( 'File type is not permitted', 'woocommerce-alidropship' ) );
			}
		} else {
			return new WP_Error( 's2w_file_type_not_permitted', esc_html__( 'Can not detect file type', 'woocommerce-alidropship' ) );
		}
	}
}
