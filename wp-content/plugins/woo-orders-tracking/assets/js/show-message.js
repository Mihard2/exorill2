 ...
 * @returns {*}
 */
function villatheme_admin_show_message(message, status = '', message_content = '', is_product_title = false, timeout = 0) {
    ...
    var timeOut = villatheme_admin_show_message_timeout(new_message, timeout);
    if (timeOut.length > 0) {
        ...
        new_message.on('mouseleave', function () {
            ...
            timeOut = villatheme_admin_show_message_timeout(new_message, timeout);
        });
    }
...
function villatheme_admin_show_message_timeout(new_message, timeout) {
    var timeOut = [];
    setTimeout(function () {
